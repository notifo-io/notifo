...
namespace Notifo.Domain.UserNotifications.Internal
{
    public sealed class StatisticsCollector
    {
        ...
        private ConcurrentDictionary<(Guid Id, string Channel), ChannelSendInfo> updates = new ConcurrentDictionary<(Guid Id, string Channel), ChannelSendInfo>();
        ...
        public async Task AddAsync(Guid id, string channel, ChannelSendInfo info)
        {
            readerWriterLock.EnterReadLock();
            try
            {
                updates[(id, channel)] = info;
            }
            finally
            {
        ...
        private async Task StoreAsync(CancellationToken ct)
        {
            ...
            var current = Interlocked.Exchange(ref updates, new ConcurrentDictionary<(Guid Id, string Channel), ChannelSendInfo>());
            ...
            List<(Guid, string, ChannelSendInfo)> commands;

            readerWriterLock.EnterWriteLock();
            try
            {
                commands = current.Select(x => (x.Key.Id, x.Key.Channel, x.Value)).ToList();
            }
            finally
            {
